...
const Shield = ({s=15,c=C.gold}: {s?: number, c?: string}) => <svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke={c} strokeWidth={1.8} strokeLinecap="round" strokeLinejoin="round"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/></svg>;
const Chk = ({s=13,c=C.grn}: {s?: number, c?: string}) => <svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke={c} strokeWidth={2.5} strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12"/></svg>;
const Xx = ({s=13,c=C.red}: {s?: number, c?: string}) => <svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke={c} strokeWidth={2.5} strokeLinecap="round" strokeLinejoin="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>;
const Ext = ({s=11,c=C.txF}: {s?: number, c?: string}) => <svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke={c} strokeWidth={2} strokeLinecap="round" strokeLinejoin="round"><path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6"/><polyline points="15 3 21 3 21 9"/><line x1="10" y1="14" x2="21" y2="3"/></svg>;
const Warn = ({s=13,c=C.amb}: {s?: number, c?: string}) => <svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke={c} strokeWidth={2} strokeLinecap="round" strokeLinejoin="round"><path d="M10.29 3.86L1.82 18a2 2 0 001.71 3h16.94a2 2 0 001.71-3L13.71 3.86a2 2 0 00-3.42 0z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg>;
const Lbl = ({children,style}: {children: React.ReactNode, style?: React.CSSProperties}) => <div style={{fontSize:10,fontFamily:F.b,fontWeight:500,color:C.txM,letterSpacing:3,textTransform:"uppercase",...style}}>{children}</div>;
const GLine = ({w=40}: {w?: number}) => <div style={{width:w,height:1,background:`linear-gradient(90deg, ${C.gold}50, transparent)`,margin:"4px 0 0"}}/>;
const TimerBadge = ({time,color=C.amb}: {time: string, color?: string}) => <div style={{display:"inline-flex",alignItems:"center",gap:4,padding:"3px 8px",borderRadius:6,background:`${color}10`,border:`1px solid ${color}12`}}><svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth={2}><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg><span style={{fontSize:10,fontFamily:F.m,color,fontWeight:500}}>{time}</span></div>;

// ─── PHONE ───
const Phone = ({ children, label, labelColor = C.txF, glow }: { children: React.ReactNode, label: string, labelColor?: string, glow?: string }) => (
...
const Bub = ({ text, from, vis = true }: { text: string, from: string, vis?: boolean }) => (
...
const SysCard = ({ children, color = C.gold, vis = true }: { children: React.ReactNode, color?: string, vis?: boolean }) => (
...
const Surf = ({ children, style, gold, onClick }: { children: React.ReactNode, style?: React.CSSProperties, gold?: boolean, onClick?: () => void }) => (
...
const Btn = ({ children, onClick, primary, style }: { children: React.ReactNode, onClick?: () => void, primary?: boolean, style?: React.CSSProperties }) => (
...
function WithoutTally({ scenario, step }: { scenario: string, step: number }) {
  if (scenario === "first") {
...
function WithTally({ scenario, step }: { scenario: string, step: number }) {
  if (scenario === "first") {
    return <div style={{ flex: 1, display: "flex", flexDirection: "column" }}>
      ...
      {done && <div style={{ marginTop: 28, display: "flex", gap: 10, justifyContent: "center" }}>
        {(scIdx === 0 ? [["Time","8–15 min","~30 sec"],["Switches","4","0"],["Card exposed","Yes","Never"],["Steps","10","3"]] : scIdx === 1 ? [["Effort","Manual","Zero"],["Switches","2","0"],["Time","1–2 min","0 sec"]] : [["Setup","Full redo","1 tap"],["Cards re-entered","Yes","No"],["Friction","Maximum","Zero"]]).map(([l,b,a],i) => (
          <div key={i} style={{ flex: 1, maxWidth: 180 }}>
            <div style={{ fontSize: 9, color: C.txF, fontFamily: F.b, letterSpacing: 2, marginBottom: 6, textAlign: "center" }}>{(l as string).toUpperCase()}</div>
...
// ═══════════════════════════════════════════════
const Kw=({children}: {children: React.ReactNode})=><span style={{color:"#C792EA"}}>{children}</span>;const Fn2=({children}: {children: React.ReactNode})=><span style={{color:"#FFCB6B"}}>{children}</span>;const Str=({children}: {children: React.ReactNode})=><span style={{color:"#C3E88D"}}>{children}</span>;const Cm=({children}: {children: React.ReactNode})=><span style={{color:"#546E7A"}}>{children}</span>;const Nm=({children}: {children: React.ReactNode})=><span style={{color:"#F78C6C"}}>{children}</span>;const Pr=({children}: {children: React.ReactNode})=><span style={{color:"#89DDFF"}}>{children}</span>;const Tp=({children}: {children: React.ReactNode})=><span style={{color:"#FFCB6B"}}>{children}</span>;
const Code = ({ children, label }: { children: React.ReactNode, label?: string }) => <div style={{ background: "#08080A", borderRadius: 14, padding: "16px 18px", border: `1px solid ${C.bd}`, fontFamily: F.m, fontSize: 11.5, lineHeight: 1.8, color: "#C5C0B8", whiteSpace: "pre", overflow: "auto" }}>{label && <div style={{ fontSize: 9, color: C.txF, fontFamily: F.b, letterSpacing: 2, textTransform: "uppercase", marginBottom: 8 }}>{label}</div>}{children}</div>;
